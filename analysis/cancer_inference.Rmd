...
- Model selection selects optimal ROC using 10 fold cross validation across 150 possible lambda values (equidistant on log scale from `.001` to `100`).
...
getCoefs <- function(sparse_coefs, font_size){
  require(dplyr)
  require(kableExtra)
...
model.21.coefs <- read_rds("cluster_output/model.21.coefs.rds") %>% getCoefs(14)
model.22.coefs <- read_rds("cluster_output/model.22.coefs.rds") %>% getCoefs(13)
model.2122.coefs <- read_rds("cluster_output/model.2122.coefs.rds") %>% getCoefs(12)

(roc.plot <- read_rds("cluster_output/roc_plot.rds"))

models.tune <- read_rds("cluster_output/models.bestTune.rds")

# tune_table <- tibble(model = c("21", "22", "21/22"),
...
train_model <- function(full_dat, trim, toy=T){
  ...
  train_index <- createDataPartition(full_dat$status, p = .9,
                                     list = FALSE,
                                     times = 1)
  dat_train <- full_dat[train_index, ]
  dat_test <- full_dat[-train_index, ]
  ...
  return(list(model = model, testset = dat_test))
}

model.21 <- train_model(read_rds("../data/for_R/chr21.rds"), 3)
model.21.testset <- model.21$testset
